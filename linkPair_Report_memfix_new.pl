...
sub action{
	foreach my $name (keys %read){
		...
		for (my $i = 0; $i < scalar @{$read{$name}{1}}; $i++){
			...
			for (my $j = 0; $j < scalar @{$read{$name}{2}}; $j++){
				...
				if($read{$name}{2}[$j][0] eq $read{$name}{1}[$i][0] && $read{$name}{2}[$j][3] + $read{$name}{1}[$i][3] == 16){
					if($read{$name}{1}[$i][3] == 0 && $read{$name}{1}[$i][1] - $read{$name}{2}[$j][1] < 200 && $read{$name}{2}[$j][1] - $read{$name}{1}[$i][2] < 100000){
						if($temp == -1){
							$temp = $j;
						}
						else{
					...
					if($read{$name}{1}[$i][3] == 16 && $read{$name}{1}[$i][1] - $read{$name}{2}[$j][1] > -200 && $read{$name}{1}[$i][1] - $read{$name}{2}[$j][2] < 100000){
						if($temp == -1){
							$temp = $j;
						}
						else{              
		...
		foreach my $temp (keys %local){
			...
			if($read{$name}{1}[$i][1] < $read{$name}{2}[$temp][1]){
				$G = -($read{$name}{2}[$temp][2]-$read{$name}{1}[$i][1]);
			}
			else{
				$G = -($read{$name}{2}[$temp][1]-$read{$name}{1}[$i][2]);
			}
			$G2 = -$G;

			$read{$name}{1}[$i][4][$t-2 + 1 - $FLAG] = "pos:part:$G=".$read{$name}{2}[$temp][1];
			$read{$name}{2}[$temp][4][$t-2 + 1 - $FLAG] = "pos:part:$G2=".$read{$name}{1}[$i][1];
			if($read{$name}{1}[$i][3] == 0){
